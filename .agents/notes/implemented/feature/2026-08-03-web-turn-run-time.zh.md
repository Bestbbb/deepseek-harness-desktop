...
在支持 hover 的设备上，操作附属元素按新近程度显示：最新的用户消息行与最新 Turn 尾部保持可见，每个更早的行只在 `:hover` 或 `:focus-within` 时淡入完整操作行。Turn 尾部和用户行发布显式的新近属性。[有界新近度决策](../bug-fix/2026-09-05-chat-action-recency-cost.zh.md)取代用户与 steering 行的后继兄弟 CSS 机制；每个已挂载消息都无需订阅并反向扫描聚合 Chat snapshot。触屏设备保持每条操作行可见，opacity 则保持布局稳定。
...
**在每个消息 renderer 内计算最新的用户消息行。** 不采用：每个已挂载行都会订阅聚合 Chat snapshot，并在任意 Chat 值变化时反向扫描 order。ChatView 为 order 统一计算一次新近度；CSS 通过得到的行属性控制可见性。
