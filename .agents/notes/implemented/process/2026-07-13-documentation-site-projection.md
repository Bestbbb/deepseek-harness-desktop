...
`scripts/project-doc-site.ts` projects the manifest into the ignored `website/.generated/` directory before VitePress starts or builds. Each page receives its canonical `editSource` and fully resolved `editSourceUrl` in frontmatter. VitePress serializes edit-link callbacks without module closures, so the callback reads only page data. [The community repository identity](../../../../website/repository.ts) supplies source, asset, and edit origins.

Locale home projections retain the canonical landing-page content and remove the source-only language switcher. [The desktop home](../../../../docs/user/index.md) owns its bilingual copy, real product screenshots, and links. The VitePress theme owns presentation, while the existing locale routes, documentation navigation, search, and theme controls remain available. Home projection tests reject the obsolete redirect-only output.

The projector parses Markdown links without reserializing the document. A link to another published source becomes a site-relative route; a link to an unpublished repository file becomes a source link under the community repository home; a repository image is copied into the generated tree and referenced from there ([why](2026-08-06-doc-site-carries-its-images.md)). Missing relative targets fail projection. Unit tests pin these transformations, and `docs:check` runs the projector tests plus a production VitePress build as part of `doc-sync` and the parallel documentation gates.
...
Mermaid renders the canonical diagrams. The theme registers its component asynchronously, and the Vite configuration retains the plugin's configuration and resolution hooks without its eager app-entry import. Pages without diagrams do not load the Mermaid runtime. The theme uses system fonts without downloading Inter. The website workspace explicitly declares Vue and the five packages that `vitepress-plugin-mermaid` asks Vite to prebundle because pnpm's strict dependency isolation otherwise makes those transitive packages unavailable to the local development server.
