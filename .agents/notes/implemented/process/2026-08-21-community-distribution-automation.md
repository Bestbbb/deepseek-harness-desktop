...
The primary CI workflow replaces the upstream enterprise runner matrix with one comprehensive keyless aggregate on the standard GitHub-hosted `ubuntu-latest` runner. It runs on pull requests, pushes to `main`, and explicit dispatches, uses bounded gate and test concurrency for the smaller public runner, and executes `pnpm run check:ci`. The separate Desktop workflow owns target-native macOS and Windows tests and packaging.
