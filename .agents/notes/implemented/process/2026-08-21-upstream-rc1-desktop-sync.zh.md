...
[手动浏览器基准测试](../../../../apps/web/tests/complex-history.perf.ts)遵循上游的紧凑侧栏控制，以及 Chat/Trajectory 各自的历史窗口。测试把 fixture 数量与实际挂载的 DOM 行分开验证，因此分页和虚拟化不要求缩小负载或一次渲染全部记录。测试主体与清理失败均保持可见。其 Chromium 计时与堆报告补充桌面验证，但不能证明原生 WebView 性能，也不能替代带凭据的真实模型任务。
