...
在 VitePress 启动或构建之前，`scripts/project-doc-site.ts` 会把 manifest 投影到被忽略的 `website/.generated/` 目录。每个页面的 frontmatter 包含权威源路径 `editSource` 和完整解析后的 `editSourceUrl`。VitePress 序列化编辑链接回调时不保留模块闭包，因此回调只读取页面数据。[社区仓库身份](../../../../website/repository.ts)提供源文件、资源和编辑链接的源站。

各 locale 的首页投影保留权威落地页正文，只移除面向源文件的语言切换行。[桌面首页](../../../../docs/user/index.zh.md)拥有双语文案、真实产品截图及链接，VitePress 主题负责呈现，现有 locale 路由、文档导航、搜索和主题控件保持可用。首页投影测试会拒绝陈旧的仅重定向输出。

投影器解析 Markdown 链接，但不会重新序列化文档。指向另一个已发布源文件的链接会变成站内相对路由；指向未发布仓库文件的链接会变成社区仓库主页下的源文件链接；仓库图片会被拷贝进生成树并从那里引用（[原因](2026-08-06-doc-site-carries-its-images.zh.md)）。相对目标不存在时，投影会失败。单元测试会锁定这些转换行为，`docs:check` 则运行投影器测试和 VitePress 生产构建，并将二者纳入 `doc-sync` 和并行文档门禁。
...
Mermaid 渲染权威图表。主题异步注册图表组件，Vite 配置保留插件的配置和解析钩子，但不使用其应用入口的提前导入。不含图表的页面不会加载 Mermaid 运行时。主题使用系统字体，不下载 Inter。网站工作区显式声明 Vue 和 `vitepress-plugin-mermaid` 要求 Vite 预打包的 5 个包，因为 pnpm 的严格依赖隔离会使本地开发服务器无法使用这些传递依赖。
