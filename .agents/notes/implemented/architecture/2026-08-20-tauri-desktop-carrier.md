...
Native dependency rebuilding uses npm's installed-tree traversal for an explicit approved package list. The legacy pnpm deployment retains workspace importer identifiers and an empty root importer, so `pnpm rebuild` at the deployment root does not reach those dependencies. Node-gyp receives the bundled Node version and architecture explicitly, rather than inheriting the builder's ABI; preparation verifies native loading with the bundled executable. This covers `fs-ext`, whose Session-lock binding is ABI-specific. A foreign runtime target is rejected before output replacement because the deployment has no cross-platform native-addon build path.
