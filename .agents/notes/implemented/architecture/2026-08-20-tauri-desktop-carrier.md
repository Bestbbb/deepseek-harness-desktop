...
The build environment assigns the pinned compiler, Node version, and architecture to every inherited casing of its owned npm fields. The lifecycle runner merges the ambient environment again, so retaining those spellings with matching values prevents Windows' case-insensitive lookup from selecting a stale compiler or ABI target. Unrelated environment fields remain unchanged.
