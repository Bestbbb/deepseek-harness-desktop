...
`apps/desktop-runtime` 是生产部署根。它的 manifest 显式闭合 workspace 运行时依赖图，避免 `pnpm deploy --prod --legacy` 静默遗漏只通过 peer 引入的 Harness 包。`scripts/verify-runtime-closure.ts` 校验该 manifest；`apps/desktop/scripts/prepare-runtime.mjs` 校验官方 Node 归档的 checksum，并写入运行时清单。每个目标操作系统自行构建运行时与安装包，不在平台之间复制二进制文件。
...
回环地址不是权限边界。每次启动都会生成一枚密码学随机 Web token，并以不可枚举属性注入 WebView。浏览器 HTTP 请求携带 `x-dsh-desktop-token`，WebSocket 携带 `dsh-auth.<token>` subprotocol；Connection host 在分发前拒绝缺失或错误 token。TypeScript 到 Rust 的私有 bridge 使用另一枚随机 token，允许的操作仅为状态、显示、通知和开机启动。普通 Web 部署不设置 token，行为保持不变。
...
- 本地 macOS 构建使用 ad-hoc 身份，使嵌套代码与 bundle resources 的签名一致，但仍是未公证的开发产物。公开发布仍需所有者提供 Apple/Windows 签名身份、公证配置和更新签名材料。secret 永不提交；本地产物不是公开发布候选。
