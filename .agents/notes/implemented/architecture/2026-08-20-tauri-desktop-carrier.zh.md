...
产物构建完成后，[社区 CI](../../../../.github/workflows/ci.yml)会在 Chromium 中以只读 `DSH_SNAPSHOT=replay` 模式执行桌面组合回放。浏览器安装命令在声明 Playwright 依赖的 Web 前端工作区内执行；根工作区不提供该可执行文件。CI 也执行工作流策略测试，确保回放步骤存在，并禁止社区 issue-policy 占位工作流引用上游专属凭据。仅本地回放无法在后续同步上游时发现桌面覆盖层回归；这两项检查都不能替代原生 WebView 验收。
