...
1. Fix the publication model and maintainer ownership. The proposed default is GitHub PR review and static hosting, with no paid server or publisher login system. Define who can approve code, publish catalogs and respond to unsafe releases; collect the owner's choice before enabling an external trust source.
2. Add a trusted remote catalog provider. Bind each review confirmation to catalog revision, target version and artifact digest, not only an entry ID. Validate bounded HTTPS downloads, redirect policy, expiry, replay protection, cached/offline behavior and artifact identity. Define signing-key custody and rotation before publishing keys; a checksum beside an untrusted download is not authentication.
