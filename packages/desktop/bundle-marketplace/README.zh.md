...
- 目录由部署方提供并保存在本地；[源码提案](../../../docs/cookbook/desktop-marketplace-bundle.zh.md)需要维护者审核并随桌面发行版发布。不提供远程下载、自助发布或自动更新。
- 已安装版本来自磁盘读取，而非运行健康检查。使用与配置入口需要插件明确贡献。登录表单、远程更新列表及活动任务的重启协调仍是独立工作。卸载保留旧代际和插件数据；不提供存储清理。
