...
“发现”与“版本变更”在本地搜索标题、包名、发布者和当前语言的描述，不增加 Host 读取。“版本变更”仅包含已知安装版本与目录不同的条目，保留兼容性警告及现有确认流程。搜索和视图变化会关闭已打开的确认。清单不可读时，不会给出误导性的无差异结论。

准备服务配置在线频道时，“检查在线目录”显式刷新其签名元数据。标签页区分内置、已验证在线、缓存和不可用元数据；单纯刷新状态不会联系目录服务器。目录检查不安装代码，也不需要平台账号。安装携带条目的审核令牌，元数据变化后必须重新确认。

“操作记录”按需读取有界准备日志，与发现独立。已准备表示其收据通过校验，不代表原生排队、启动或插件健康检查成功。未结算结果、准备失败、元数据不可读及收据缺失或变化保持区分。未配置或不可读的历史显示不可用，不会变成空列表。连接替换和刷新会丢弃陈旧响应。不提供后台轮询或历史更改操作。
...
- 未配置可信在线频道时，打包目录保存在本地。[源码提案](../../../docs/cookbook/desktop-marketplace-bundle.zh.md)需要维护者审核；本 UI 不提供公开签名目录托管或自助发布，不会自动安装。
- 已安装版本来自磁盘读取，而非运行健康检查。使用与配置入口需要插件明确贡献。登录表单、公开目录运营及活动任务的重启协调仍是独立工作。卸载保留旧代际和插件数据；不提供存储清理。
- 桌面目录提供可选的 [Focus Timer](../focus-timer/README.zh.md)、[Notification Controls](../notification-controls/README.zh.md) 和 [Delegate Tasks](../delegation-launcher/README.zh.md)，均有“已安装”使用入口。这是经过筛选的第一方目录，不是开放的发布者市场。
