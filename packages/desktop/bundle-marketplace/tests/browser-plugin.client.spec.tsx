...
describe('marketplace browser registration', () => {
  it('mounts its own Remote and retracts tab, namespace and dictionary on disposal', async () => {
    ...
    ctx.provide('remote', { $mount: mount, bundleMarketplace: { snapshot, queueActivation: install, queueRemoval: remove, cancel, openLocalAgents } } as never)
    ctx.provide('remote.bundleMarketplace', { snapshot, queueActivation: install, queueRemoval: remove, cancel, openLocalAgents })
