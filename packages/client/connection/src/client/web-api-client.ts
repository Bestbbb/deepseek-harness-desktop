...
import { desktopAuthProtocols, withDesktopAuth } from '../desktop-auth.ts'
...
export class WebApiClient extends AbstractApiClient {
  protected doFetch(input: URL, init?: RequestInit): Promise<Response> {
    return globalThis.fetch(input, { ...init, headers: withDesktopAuth(init?.headers) })
  }
  ...
  private async *readWebSocket<F extends MuxFrame | HostFrame>(
    ...
    const protocols = desktopAuthProtocols()
