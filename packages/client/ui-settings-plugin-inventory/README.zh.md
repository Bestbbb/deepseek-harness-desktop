...
**刷新状态**会重新读取宿主，不改变配置、搜索或展开选择。刷新失败会保留上次结果，同时明确提示数据已过期并提供重试入口；传输细节保持隐藏。连接断开时隐藏该连接代次的条目，新连接建立后重新读取。过期请求的迟到响应不能替换当前结果。清单不使用定时轮询。

配置已启用不等于插件正在运行或账号已登录。预设卡片详情分别展示配置与观测到的运行阶段，启动失败时也不混淆二者。根 Fiber 缺失显示为**未观测到运行实例**，不代表所有实例都已停止。
...
- **即时观测**：标签页在挂载、刷新、重试和连接更换时读取，不逐项订阅 Loader 变化。切换标签页会保留当前结果。这份运行时清单不能证明包已安装、版本兼容、账号就绪或插件权限。
