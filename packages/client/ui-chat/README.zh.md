...
用户和 steering 操作行的新近度由 ChatView 在 order 改变时统一计算。悬停和聚焦使用本地行属性，不执行后继兄弟搜索；全部已加载历史保持挂载（[开销决策](../../../.agents/notes/implemented/bug-fix/2026-09-05-chat-action-recency-cost.zh.md)）。
