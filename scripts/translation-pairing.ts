...
function isTranslationSourceExcluded(file: string): boolean {
  ...
  return segments.some(segment => NON_SOURCE_DIRECTORIES.has(segment)
    ...
    || file.startsWith('apps/desktop/resources/runtime/')
    || file.startsWith('apps/desktop/.runtime-cache/')
    || file.startsWith('apps/desktop/src-tauri/target/')
