...
Slots 是 Web Client 的类型化 React 组合系统。[`dsh-client-ui-slots`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/client/ui-slots/README.zh.md)定义不依赖 React 的注册表与类型代数；[`dsh-client-ui-renderer`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/client/ui-renderer/README.zh.md)把可观测源绑定成钩子、渲染整棵树，并在内部拥有 React context。功能插件通过 `ctx.slots.register()` 贡献 UI，绝不导入其他功能插件的组件。
