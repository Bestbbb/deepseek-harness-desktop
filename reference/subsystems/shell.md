...
bash 执行 seam 分为 Service Definition（[dsh-shell](https://github.com/Bestbbb/deepseek-harness-desktop/tree/main/packages/shell/shell)，`ctx.shell`）、Service Provider（[dsh-bash-local](https://github.com/Bestbbb/deepseek-harness-desktop/tree/main/packages/shell/bash-local) 与 [dsh-bash-sandbox](https://github.com/Bestbbb/deepseek-harness-desktop/tree/main/packages/shell/bash-sandbox)）和 Consumer（[dsh-tool-bash](https://github.com/Bestbbb/deepseek-harness-desktop/tree/main/packages/shell/tool-bash)，即 `bash` schema）。通用后台任务的 job id、所有权与控制位于 [jobs.md](./jobs.md)；本 seam 返回一个不含任务概念的进程句柄。原始进程组机制封装在[子进程 seam](./subprocess.md)之后。

源码：[`packages/shell/shell/src/types.ts`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/shell/shell/src/types.ts)
...
`stdin` 和 `env` 是受信任的进程内插件输入，不由 `dsh-tool-bash` 暴露。本地执行器会先清除环境中的凭据，再合并调用方显式提供的 env。见 [bash-stdin-env Agent Note](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/.agents/notes/implemented/architecture/2026-06-30-bash-stdin-env-trusted-plugin-api.zh.md)。
