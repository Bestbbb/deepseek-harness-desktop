...
Windows 以暂停且不显示控制台窗口的方式创建运行时，在分配到 Job 后才恢复执行。启动过程报告失败时，会先终止并回收子进程再重试；退出时会等待 Job 的活动进程数归零。如果在创建进程到分配 Job 之间强行终止宿主，仍可能留下暂停的子进程；这段启动过程不是原子进程创建。[桌面生命周期决策](../../.agents/notes/implemented/architecture/2026-08-20-tauri-desktop-carrier.zh.md)记录了各平台的验证要求。
