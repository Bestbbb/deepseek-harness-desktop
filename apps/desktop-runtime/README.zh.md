...
这个 workspace package 是桌面应用所捆绑 TypeScript 运行时的部署根，本身不含应用逻辑。它的依赖闭合了从 CLI（命令行界面）、Web 前端和 desktop-native provider 可达的生产 workspace 依赖图，其中包括旧版 `pnpm deploy` 可能遗漏的必需对等依赖（peer dependency）。
