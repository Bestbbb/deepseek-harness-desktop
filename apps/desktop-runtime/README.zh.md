...
这个 workspace package 是桌面应用所捆绑 TypeScript 运行时的部署根，本身不含应用逻辑。它的依赖闭合了从 CLI（命令行界面）、Web 前端、desktop-native provider、Bundle 准备服务和精选 Codex／Claude Code／ACP 适配器可达的生产 workspace 依赖图，其中包括旧版 `pnpm deploy` 可能遗漏的必需对等依赖（peer dependency）。打包插件不等于启用它；准备服务需要显式覆盖层，适配器的启用选择由桌面扩展偏好管理。
...
仅通过可选依赖到达的包及其必需下游依赖保留在 `optionalDependencies` 中。如果另有必需路径到达同一个包，该包及其必需下游依赖会提升到 `dependencies`。这会保留平台专属的可选包，而不要求每个操作系统都安装所有原生二进制文件。
