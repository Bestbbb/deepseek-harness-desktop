...
To use a fresh source checkout instead, complete the [run-from-source section](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/README.md#run-from-source), keep this tutorial's `hello-plugin` directory at the repository root, and run the remaining `dsh ...` commands from there as `pnpm dsh ...`. See [source execution](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/apps/cli/reference/README.md#source-execution) for build and launcher behavior.
...
- Your patch can override rows from earlier layers by `id` — the same way [the `dsh-web-app` bundle](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/web-app/cordis.patch.yml) overrides `dsh-base` rows — but must restate every key the row needs, not just the changed one.
...
- [CLI behavior reference](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/apps/cli/reference/README.md) — exact layer precedence, flags, and profile mechanics
