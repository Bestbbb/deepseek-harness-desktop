...
[`dsh-base`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/base/README.md) is the shared first layer of the `web`, `headless`, `sdk`, and `acp` profiles: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/web-app/README.md) adds the browser application, [`dsh-headless`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/headless/README.md) adds a one-shot runner with no server, [`dsh-sdk-app`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/sdk-app/README.md) adds the SDK JSON-RPC server, and [`dsh-acp-app`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/acp-app/README.md) adds the automation-only ACP server. [`dsh-sdk-minimal`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/sdk-minimal/README.md) is the deliberate exception: one bundle owns its complete explicit SDK tree and does not apply `dsh-base`.
...
Composition mechanics are in [app-boot](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/boot/app-boot/README.md#profiles); config fields are in the generated [config catalog](./reference/config-catalog.md).
...
Vendored CLIs, build-only and test-only executables, direct in-process plugin mounting, and the private browser WebWorker preview are not Harness application launchers. [`verify-application-entrypoints`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/scripts/verify-application-entrypoints.ts) keeps every package bin, executable source, and root demo in an explicit class and rejects a Node application path that bypasses `dsh`.
...
|---|---|---|
| [`core/session`](./reference/subsystems/session.md) | The append-only `SessionEvent` log and in-memory store | `ctx.sessions` |
| [`core/system-prompt`](./reference/subsystems/system-prompt.md) | Prompt-section and tool-schema assembly | `ctx.systemPrompt` |
| [`core/tools`](./reference/subsystems/tools.md) | The scoped tool registry and guarded execution pipeline | `ctx.tools` |
| [`core/agent`](./reference/subsystems/core.md) | The `Agent` interface, live registry, and `agent/*` events | `ctx.agents` |
...
| [`webhook/webhook`](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/docs/subsystems/webhook.md) | Authenticated-delivery dispatch and Workspace Session creation | `ctx.webhookRuntime` |
...
The [event map](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/docs/event-producer-consumer.md) lists every event's producers and consumers.
...
[Experimental Agent Teams](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/docs/subsystems/agent-team.md) is a private opt-in coordination seam on `ctx.agentTeams`, with a durable roster, task board, and mailbox layered over continuable subagents.
