...
真实 agent 就是这套组合再加上更多插件：LLM（大语言模型）适配器、agent loop（智能体循环）、持久化和应用入口。对照 [base profile 层](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/base/cordis.patch.yml)与 [headless 层](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/packages/bundle/headless/cordis.patch.yml)，你现在已经可以读懂其中各项。通过一个小型 `--patch` overlay 加入 `greet-tool.ts` 即可。
