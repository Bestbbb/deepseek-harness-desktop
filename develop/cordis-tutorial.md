...
你需要克隆本仓库并安装依赖；[开发指南](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/docs/development.zh.md#setup-tutorial)列出了前置条件。本教程不需要 API 密钥；所有示例均可在无密钥环境中运行。
...
这个单文件启动器（见 [vendor/cordis/bin.js](https://github.com/Bestbbb/deepseek-harness-desktop/blob/main/vendor/cordis/bin.js)）会创建根 `Context`、挂载 Loader 插件，并让它从当前目录加载 `./cordis.yml`。其余所有内容，包括有哪些插件以及如何配置它们，都来自你稍后将编写的 YAML 文件。`--import tsx` 标志让 Node 无需构建步骤即可运行配置所指向的 TypeScript 文件。

## 章节

1. [你的第一个插件](./cordis-tutorial/01-first-plugin.md)：插件是函数，由 loader 挂载。
2. [生命周期与 effect](./cordis-tutorial/02-lifecycle-and-effects.md)：由 Cordis 管理的注册会在所属插件卸载时撤销。
3. [服务](./cordis-tutorial/03-services.md)：在 `ctx` 上公开一项能力，并通过 `inject` 依赖它。
4. [事件](./cordis-tutorial/04-events.md)：类型化事件、广播分发和 waterfall（瀑布式事件）的短路行为。
5. [配置](./cordis-tutorial/05-config.md)：读取 `cordis.yml` 中经过校验的配置，并在输入错误时明确报错。
6. [组合与 HMR（热模块替换）](./cordis-tutorial/06-composition-and-hmr.md)：把配置文件作为插件树，使用热重载，并诊断始终无法加载的插件。
7. [进入 harness](./cordis-tutorial/07-into-the-harness.md)：基于真实的 harness 服务注册一个可由模型调用的工具。
